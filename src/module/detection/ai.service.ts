  ...
  if (geminiKey) {
    try {
      ...
      if (!base64Image) {
        ...
        if (!imgRes.ok) throw new Error(`Failed to download crop image. Status: ${imgRes.status}`);
...
export interface AISimplifiedReportResult {
  summaryText: string;
  healthScore: number;
...
export const analyzeLabReport = async (
  ...
  reportType: string,
  languagePref?: string
): Promise<AISimplifiedReportResult> => {
  const geminiKey = process.env.GEMINI_API_KEY;
  ...
  if (geminiKey) {
    try {
      ...
      if (!base64File) {
        ...
        if (!fileRes.ok) throw new Error(`Failed to download report file. Status: ${fileRes.status}`);
