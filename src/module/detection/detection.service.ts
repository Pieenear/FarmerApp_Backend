...
import { CreateDetectionLogInput, UpdateDetectionLogInput } from "./detection.schema.js";
...
export const createDetectionLogService = async (
  data: CreateDetectionLogInput,
  requesterId: bigint
) => {
...
export const getDetectionLogByIdService = async (id: bigint, requesterId: bigint) => {
  const requester = await prisma.user.findUnique({
    where: { id: requesterId },
  });
  if (!requester) {
...
export const updateDetectionLogService = async (
  id: bigint,
  data: UpdateDetectionLogInput,
  requesterId: bigint
) => {
  const { farmerId, imageUrl, detectedDisease, confidenceScore, recommendation, cropType } = data;
...
export const deleteDetectionLogService = async (id: bigint, requesterId: bigint) => {
  const requester = await prisma.user.findUnique({
    where: { id: requesterId },
  });
  if (!requester) {
