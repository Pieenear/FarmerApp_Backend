...
export const createStorageUnitService = async (data: CreateStorageUnitInput) => {
...
export const listStorageUnitsService = async (areaId?: bigint) => {
...
export const getStorageUnitByIdService = async (id: bigint) => {
...
export const updateStorageUnitService = async (id: bigint, data: UpdateStorageUnitInput) => {
  const { name, location, areaId, totalCapacityKg } = data;
...
export const deleteStorageUnitService = async (id: bigint) => {
  const existing = await prisma.storageUnit.findUnique({
    where: { id },
    include: { allocations: true },
  });
  ...
  const activeAllocations = existing.allocations.filter(
    (a) => a.status !== StorageAllocationStatus.vacated
  );
...
export const createAllocationService = async (data: CreateAllocationInput, adminId: bigint) => {
  ...
  if (Number(unit.availableCapacityKg || 0) < allocatedCapacityKg) {
    throw new Error(
      `Insufficient capacity in storage unit. Available: ${unit.availableCapacityKg} kg, Requested: ${allocatedCapacityKg} kg`
    );
  }
...
export const updateAllocationStatusService = async (
  id: bigint,
